...
struct Vector(T, size_t Size)
{
  ...
  static enum isAssignableSwizzle(string Str) = !Str.canFind("_") && !Str.canFind("0") && !Str.canFind("1");
  auto swiz(string Str, Args...)(Args args)
  if (Str.count("_") == Args.length || isAssignableSwizzle!Str && Args.length == 1)
  {
    static if (isAssignableSwizzle!Str && Args.length == 1)
    {
      mixin(Str.enumerate.map!(tup => format!"this.%s = args[0].components[%s];"(tup.value, tup.index)).join);
    }
    static string prop(size_t i, dchar ch)
    {
      if (ch == '_') return format!"args[%s]"(Str[0..i].count('_'));
      if (ch == '0' || ch == '1') return ch.to!string;
      return format!"this.%s"(ch);
    }
    enum props = Str.enumerate.map!(tup => prop(tup.index, tup.value)).join(", ");
    return mixin(format!"Vector!(T, %s)(%s)"(Str.length, props));
  }

  auto simpleSwiz(string Str, Args...)(Args args) const
  {
